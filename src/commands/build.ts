...
export function register(program: Command) {
	program
		...
		.description(
			"Builds a plugin using the rollup config in the current directory"
		)
		.option("-o, --output <path>", "the file path to output the plugin to")
		.option(
			"-d, --deploy",
			"whether or not to deploy the plugin to the connected adb device"
		)
		.option(
			"-p, --package <package name>",
			"the package to use for deploying the built plugin"
		)
		.action(
			async (
				plugin: string,
				opts: { output?: string; deploy: boolean; package?: string }
			) => {
				...
				const pluginBuild = await spinner("Building plugin...", () =>
				...
				const zip = pluginBuild.outputs.find(asset =>
					asset.fileName.endsWith(".zip")
				) as OutputAsset;
				const rollupOutputPath = path.join(
					path.parse(rollupConfig.output[0].file!).dir,
					zip.fileName
				);
				...
				for (const warning of pluginBuild.warnings)
