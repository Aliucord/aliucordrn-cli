	...
	program
		...
		.action(async (pluginArg, opts) => {
			...
			const { pluginName, commitId, gitRepo } = await prompts(
				[
					...
					},
					{
						...
						name: "gitRepo",
						message:
							"What is the https git url that this plugin can be cloned from?",
					...
					},
					{
						...
						name: "commitId",
						message:
							"What is the ENTIRE commit id that this plugin can be built from?",
						validate: (t: string) =>
							t.match(/^[a-f0-9]{40}$/) !== null
								? true
								: "Please enter a valid git commit hash. The hash must be full, not shortened.",
			...
			const { confirm: confirmBuild } = await prompts({
				...
				name: "confirm",
				message:
					"To proceed, your plugin will need to be built. This can be done automatically. Proceed?",
				initial: true
			});
			...
			const { outputs: pluginBuild } = pluginBuildResult.unwrapOrElse(
				(e: unknown) => {
					console.error(e);
					process.exit(1);
				}
			...
			const pluginLicenseInfo = await spinner(
				...
				async () => {
					const spdxLicenses = (await fetch(
						`https://spdx.org/licenses/licenses.json`
					).then(res => res.json())) as SPDXLicensesInfo;
			...
			const { confirm: confirmData } = await prompts({
				...
				name: "confirm",
				message:
					"After confirming, this data will be sent and used to add your plugin to the plugins repo. Please make sure all of the information above is correct, and then proceed. Publish this data? "
			});
		});
}
